...
func (c *CompilerRego) compileStatement(stmt *ast.ActionStatement) (string, error) {
	...
	attr, err := c.compileAttr(stmt.Selector)
	if err != nil {
		return "", err
	}
	if attr != "" {
...
func (c *CompilerRego) compileAttr(selector ast.Selector) (string, error) {
	if selector == nil {
		return "", nil
	}
	...
	case *ast.WhereClause:
		return c.compileOperation(s.Conditional, 0)
	default:
...
func (c *CompilerRego) compileOperation(o ast.Condition, lvl int) (string, error) {
	...
	case *ast.BinaryCondition:
		...
		RHS, err := c.compileOperation(s.RHS, lvl+1)
		if err != nil {
			return "", err
		}
